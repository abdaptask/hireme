...
/**
 * Billing Readiness page (CLAUDE.md §16 — Payroll and Billing Readiness).
 * Validates every billing gate before a consultant's start date (§16.3).
 */
...
export default function BillingReadinessPage() {
  ...
  const filterButtons: { id: Filter; label: string; count: number }[] = [
    { id: "all", label: "All", count: BILLING_RECORDS.length },
  ...
  return (
    <PageContainer className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTile icon={AlertTriangle} label="Revenue at Risk (≤7d)" value={stats.criticalGaps} tone="danger" />
      </div>
      ...
      <section className="bg-card overflow-hidden rounded-xl border shadow-xs">
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            <tbody>
              {filtered.map((record) => (
                <BillingRow key={record.candidateId} record={record} />
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td
