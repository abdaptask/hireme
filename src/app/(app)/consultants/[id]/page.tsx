...
import { getConsultant, CONSULTANT_STATUS_META } from "@/lib/consultants";
...
export async function generateMetadata({
  ...
  return { title: c ? c.name : "Consultant" };
}
...
export default async function ConsultantRecordPage({
  ...
  // Mock assignment history (3–4 rows)
  const a1Status: AssignmentRow["status"] =
    c.status === "Active" || c.status === "Extension Pending"
      ? "Active"
      : "Completed";
  const a2Status: AssignmentRow["status"] =
    c.extensions > 0 ? "Extended" : "Completed";
  const allAssignments: AssignmentRow[] = [
    ...
    {
      id: "a2",
      client: c.assignments >= 2 ? "Vertex Financial" : c.client,
      role: c.role,
      start: "2023-04-01",
      end: "2023-12-15",
      status: a2Status,
  ...
  const assignments = allAssignments.slice(0, Math.min(c.assignments, 3));

  // Mock lifecycle timeline (4–5 events)
  const lifecycle: LifecycleEvent[] = [
    {
      id: "lc1",
      date: c.startDate,
      title: "Onboarding completed",
      detail: `Successfully onboarded to ${c.client}. Package approved by ${c.accountManager}.`,
      kind: "start",
    },
    {
      id: "lc2",
      date: "2023-09-15",
      title: "Day 30 check-in completed",
      detail: `Satisfaction score recorded: ${c.satisfactionScore}/5. Client feedback positive.`,
      kind: "milestone",
    },
    ...(c.extensions > 0
      ? [
          {
            ...
            detail: `Extension approved by ${c.accountManager}. Duration: +6 months.`,
      ...
      : []),
    {
      ...
      detail: `Bill rate updated to ${fmt(c.billRate)}. Approved by ${c.accountManager}.`,
      kind: "rate",
    },
      ...
      ? [
          {
            ...
            detail:
              c.status === "Converted"
                ? `Client conversion finalised. Conversion fee processed.`
                : `Equipment return and access removal in progress.`,
            kind: "end" as const,
          },
        ]
      : []),
      ...
      <PageContainer className="flex flex-col gap-5 pt-5">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatTile
            icon={Star}
            label="Satisfaction"
            value={c.satisfactionScore.toFixed(1)}
            suffix="/ 5"
            tone="success"
          />
        </div>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="mt-4">
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-card rounded-xl border p-4 shadow-xs">
                  ...
                  <InfoRow
                    ...
                    label="Bill rate"
                    value={
                      <span className="tabular-nums">{fmt(c.billRate)}</span>
                    }
                  />
                  <InfoRow
                    icon={DollarSign}
                    label="Pay rate"
                    value={
                      <span className="tabular-nums">{fmt(c.payRate)}</span>
                    }
                  />
                  <InfoRow
                    icon={Briefcase}
              ...
              {/* Right: tags + extensions + satisfaction + notes */}
              <div className="flex flex-col gap-4">
                {c.tags.length > 0 && (
                  <div className="bg-card rounded-xl border p-4 shadow-xs">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-metadata mt-0.5">
                        Based on {c.assignments} assignment
                        {c.assignments !== 1 ? "s" : ""}
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-card rounded-xl border p-4 shadow-xs">
                  <h3 className="text-card-heading mb-2">Internal notes</h3>
                  <p className="text-muted-foreground text-sm italic">
                    No notes added yet. Internal notes are visible only to
                    authorized staff and are never shown to the consultant or
                    client.
                  </p>
                </div>
              </div>
                ...
                >
                  <thead>
                    <tr className="text-muted-foreground border-b">
                      {["Client", "Role", "Start", "End", "Status"].map(
                        (h, i) => (
                          <th
                            key={i}
                      ...
                      <div className="flex flex-col items-center">
                        <span
                          className={cn(
                            ...
                            LIFECYCLE_COLORS[ev.kind],
                          )}
                        >
                          <Icon className="size-4" />
                        </span>
                        {!last && (
                          <span className="bg-border my-1 w-0.5 flex-1" />
                        )}
                      </div>
