 ...
 * Validates every payroll gate before a consultant's start date (§16.2).
 */
...
import type { PayrollRecord, PayrollReadinessStatus } from "@/lib/payroll";
...
export default function PayrollReadinessPage() {
  ...
  const filterButtons: { id: Filter; label: string; count: number }[] = [
    { id: "all", label: "All", count: PAYROLL_RECORDS.length },
      ...
      <section className="bg-card overflow-hidden rounded-xl border shadow-xs">
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            <tbody>
              {filtered.map((record) => (
                <PayrollRow key={record.candidateId} record={record} />
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td
