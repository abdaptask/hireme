...
import type { CommChannel, CommunicationRecord } from "@/lib/communications";
...
function statsFor(rows: CommunicationRecord[]): {
  sent: number;
  delivered: number;
  opened: number;
  failed: number;
  ...
  return {
    sent: rows.length,
    delivered: rows.filter((c) =>
      ["delivered", "opened", "replied"].includes(c.status),
    ).length,
    opened: rows.filter((c) => ["opened", "replied"].includes(c.status))
      .length,
    failed: rows.filter((c) => ["failed", "bounced"].includes(c.status))
      .length,
  };
}
...
export function CommunicationsClient({
  ...
  const stats = useMemo(() => statsFor(communications), [communications]);
