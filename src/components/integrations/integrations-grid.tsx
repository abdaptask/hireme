...
import type { Integration, IntegrationCategory, IntegrationStatus } from "@/lib/integrations";
...
export function IntegrationsGrid({ integrations, dbIds }: Props) {
          ...
          const successBarColor =
            integration.successRate >= 95
              ...
              : integration.successRate >= 80
                ? "bg-warning"
                : "bg-danger";
          const isLive = dbIds.has(integration.id);

          return (
            <article
              key={integration.id}
              className={cn(
                "bg-card flex flex-col rounded-xl border shadow-xs transition-shadow hover:shadow-sm cursor-pointer",
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 border-b px-4 py-3">
                <div className="min-w-0">
                  <h3 className="text-card-heading flex items-center gap-1.5 truncate">
                    {integration.name}
                    {isLive && <span className="text-success text-[10px] font-medium">●</span>}
                  </h3>
                  <p className="text-metadata mt-0.5">
                    {integration.vendor} · {CATEGORY_META[integration.category]?.label ?? integration.category}
                  </p>
                </div>
                <div className="flex shrink-0 flex-col items-end gap-1.5">
                  <span className="inline-flex items-center gap-1.5 text-xs font-medium">
                    <span className={cn("size-2 shrink-0 rounded-full", STATUS_DOT[integration.status])} />
                    {statusMeta.label}
                  </span>
                  <span className={cn("rounded-full px-2 py-0.5 text-xs font-medium", CRITICALITY_TONE[integration.criticality])}>
                    {integration.criticality.charAt(0).toUpperCase() + integration.criticality.slice(1)}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-1 flex-col gap-3 px-4 py-3">
                ...
                <div>
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <span className="tabular-nums text-xs font-semibold">
                      {isHealthy || integration.successRate > 0 ? `${integration.successRate}%` : "—"}
                    </span>
                  </div>
                  <div className="bg-muted h-1.5 overflow-hidden rounded-full">
                    <div className={cn("h-full rounded-full", successBarColor)} style={{ width: `${Math.max(integration.successRate, 0)}%` }} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className={cn("tabular-nums text-sm font-semibold", integration.failedRecords > 0 ? "text-danger-muted-foreground" : "text-success-muted-foreground")}>
                      {integration.failedRecords}
                    </p>
                  </div>
                  <div>
                    <p className="text-metadata">Queue</p>
                    <p className={cn("tabular-nums text-sm font-semibold", integration.queueDepth > 5 ? "text-warning-muted-foreground" : "")}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-metadata truncate">
                    {integration.lastSync !== "—" ? `Last sync ${integration.lastSync.split(" ")[1]}` : "Not synced"}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2 border-t px-4 py-2.5">
                <button
                  ...
                  className={cn(
                    ...
                    isHealthy ? "text-muted-foreground/50 cursor-not-allowed" : "bg-primary text-primary-foreground hover:bg-primary/90",
                  )}
                >
